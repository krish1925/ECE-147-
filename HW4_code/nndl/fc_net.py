...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores, fc_cache2 = affine_forward(h, W2, b2)
    ...
    dh, dW2, db2 = affine_backward(dscores, fc_cache2)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
        if i == 0:
            ...
            caches.append(cache)
            if self.use_batchnorm:
                a, cache = batchnorm_forward(a, self.params['gamma1'], self.params['beta1'], self.bn_params[0])
                caches.append(cache)
            ...
            scores, cache = affine_forward(h, self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
            caches.append(cache)
        else:
            a, cache = affine_forward(h, self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
            caches.append(cache)
            if self.use_batchnorm:
                a, cache = batchnorm_forward(a, self.params['gamma' + str(i + 1)], self.params['beta' + str(i + 1)], self.bn_params[i])
                caches.append(cache)
            h, cache = relu_forward(a)
            caches.append(cache)
            if self.use_dropout:
    ...
    dh, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = affine_backward(dscores, caches.pop())
